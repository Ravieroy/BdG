    ...
    for alpha in alphaList
        ...
        for V in vVals
            ...
            for T in tempList
                nJob += 1
                #------Logs-------
                write_log(logFileName, status="Running", alpha=alpha, V=V, T=T, nJob=nJob, nt=nTotalJobs)
                vList = vDict[seed][V]
                deltaFinal, nAvgFinal, eGap, deltaGap, evecs, evals, count, endTime, isConverged =
                     run_self_consistency_numpy(T, deltaOld, mu, N, nAvgOld, nExp, lattice, tMat, U, vList, tol=tol, nLayer=nLayer, maxCount=maxCount)
                if isConverged == true
                    write_log(logFileName, status="Converged", count=count, endTime=endTime)
                else
                    write_log(logFileName, status="HitMaxCount", count=count, endTime=endTime)
                end
...
println("($dateToday|$timeNow)The elapsed time : $elapsed secs ($(round(elapsed/60, digits = 2)) mins)")
