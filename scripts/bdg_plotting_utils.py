...
def plot_probability_distribution(info_dict):
    ...
    for i in range(len(alpha_list_plot)):
        for j in range(len(v_vals_plot)):
            dd = dict_[alpha_list_plot[i]][v_vals_plot[j]][T]
            sns.kdeplot(np.absolute(dd), ax=ax,
...
def plot_gap(info_dict):
    ...
    T = info_dict.T
    for alpha in alpha_list_plot:
        egap_list = list(avg_egap_dict_full[alpha][T].values())
        δgap_list = list(avg_delta_gap_dict_full[alpha][T].values())
        if np.iscomplex(δgap_list).any():
            print("WARNING : δ-gap contains complex entries.Ignored for plotting")
        δgap_list = np.array(list(δgap_list)).real
        plt.plot(v_vals_plot, egap_list,
                 '^-', label=r"$E_{gap}/t$")
        plt.plot(v_vals_plot, δgap_list,
